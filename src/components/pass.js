...
function setPassword(e) {
  ...
  fetch(
    `${process.env.REACT_APP_backEndAPI_URL}/pass/setpassword`,
    requestOptions
  )
    ...
    .then((result) => {
      if (result.status == 200 && result.token) {
        ...
        document.getElementById("message").innerHTML =
          "Password was not set, the link might have expired. Please try again.";
      ...
      .then(
        (result) => {
          ...
          if (result.status == 404) {
            ...
            document.getElementById("message").style.color =
              passMessagesColor[messageNumber];
          } else {
  ...
  return (
    <div className={classes.body}>
...

      {
        <h3
          id={"message"}
          style={{ color: `${passMessages[messageNumber]}` }}
        ></h3>
      }
      {(isLoaded && (
        <div id="frame" className={classes.signContainer}>
          <form id={"form"}>
            ...
            <div>
              ...
              <input
                type="password"
                placeholder="enter desire password"
                id="3"
                name="password"
                required
                className={classes.signInput}

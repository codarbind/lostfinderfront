...
export default function Dashboard(props) {
 ...
 function decide(e){
  ...
  let responseIfDecisionAcceptedForOwnerReporter = (props.details.owner && (props.details.reporter) && (props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{ color: 'white' }}>Congrats. You can contact the finder of your item:{props.owner}</h3></span>));
  let responseIfDecisionAcceptedForNotOwnerNotReporter = (!props.details.owner && !(props.details.reporter) && (props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{ color: 'white' }}>Superb!! The owner of the item shall get in touch with you via: {props.reporterOfItem}</h3></span>));
  let responseIfDecisionAcceptedForOwnerNotReporter = (props.details.owner && !(props.details.reporter ) && (props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>Cheers on finding your item. Contact the finder of your item on:{props.owner}</h3></span>) );
  let responseIfDecisionAcceptedForNotOwnerReporter = (!props.details.owner && (props.details.reporter ) && (props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>We are grateful. The owner of the item shall get in touch with you. You call also reach them via: {props.reporterOfItem}</h3></span>) );

  let responseIfDecisionRejectedForNotOwnerReporter = (!props.details.owner && (props.details.reporter ) && (props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>Great, we shall let this user know that the item with you is not theirs. Thank you for your help.</h3></span>) );
  let responseIfDecisionRejectedForNotOwnerNotReporter = (!props.details.owner && !(props.details.reporter ) && (props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>Marvelous!! We shall let this user know that the item is not yours. Best of luck in finding yours.</h3></span>) );
  let responseIfDecisionRejectedForOwnerReporter = (props.details.owner && (props.details.reporter ) && (props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>Awesome Still!! We shall let the finder of this item know that it is not yours. Best of luck in finding yours.</h3></span>) );
  let responseIfDecisionRejectedForOwnerNotReporter = (props.details.owner && !(props.details.reporter ) && (props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>Hmmm, the finder thinks the item is not yours. We will keep looking out for yours.</h3></span>) );
  
  let responseIfDecisionRejectedIndirectForNotOwnerReporter = (!props.details.owner && (props.details.reporter ) && (props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>The user did not accept this item with you as not theirs. Kindly report this item found to enable rightful owner find it. Thank you for your help.</h3></span>) );
  let responseIfDecisionRejectedIndirectForNotOwnerNotReporter = (!props.details.owner && !(props.details.reporter ) && (props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>Nothing spoil!! The owner of the report has found their true item. Please report this item you found to enable the owner find it. We appreciate your support.</h3></span>) );
  let responseIfDecisionRejectedIndirectForOwnerReporter = (props.details.owner && (props.details.reporter ) && (props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>Wow!! We shall let the finder of this item know that it is not yours. We are happy you have found yours.</h3></span>) );
  let responseIfDecisionRejectedIndirectForOwnerNotReporter = (props.details.owner && !(props.details.reporter ) && (props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>Ermmm, it seems your description did not match the item, as the item has been returned to someone else. Kindly report your item missing. Let's find it.</h3></span>) );

  let responseIfDecisionRejectedForNotOwnerReporterUnsettled = (!props.details.owner && (props.details.reporter ) && !(props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>Respect!! They do not think the item is theirs. Please try and report it on this platform. Let us find the owner together, cheers!</h3></span>) );
  let responseIfDecisionRejectedForNotOwnerNotReporterUnsettled = (!props.details.owner && !(props.details.reporter ) && !(props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>We are grateful!! However, the other user do not think the item with you is theirs. Kindly report this item found, let us find the rightful owner.</h3></span>) );
  let responseIfDecisionRejectedForOwnerReporterUnsettled = (props.details.owner && (props.details.reporter ) && !(props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>Lets keep looking... We have updated the other user that that is not your item.</h3></span>) );
  let responseIfDecisionRejectedForOwnerNotReporterUnsettled = (props.details.owner && !(props.details.reporter ) && !(props.status == 'settled') && (<span><hr /><span style={{float:'right'}}>UPDATE</span><h3 style={{color:'white'}}>Hmmm, the finder thinks the item is not yours. We will keep looking out for yours.</h3></span>) );
